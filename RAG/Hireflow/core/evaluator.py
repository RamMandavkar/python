...
class RAGEvaluator:
    ...
    def evaluate_with_ragas(self, query: str, candidates: List[Dict], 
        ...
        metrics = RAGEvaluationMetrics(
            answer_relevancy=_safe(results.get('answer_relevancy', 0.0)),
            context_precision=_safe(results.get('context_precision', 0.0)),
            faithfulness=_safe(results.get('faithfulness', 0.0)),
            answer_correctness=_safe(results.get('answer_correctness', 0.0)),
            overall_score=0.0
        )
